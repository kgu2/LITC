...
export default class LwcDatatableUtility extends OmniscriptBaseMixin(LightningElement)   {
    ...
    @api preSelectedItems=[];
    @api mode;
    @track hideCheckbox = false;
    tableHeightStyle = 'height: '+ DEFAULTHEIGHT +'px;';    // Set Default Height as 300px 
    noMatches = false; //Boolean to be used for displaying text when search results in zero matches.
    @api
    get tableHeight() {
    ...
    @track preSelected; //preSelectedOnDisplay
    ...
    connectedCallback() {
        ...
        // console.log('this.preSelectedItems passed from parent to child:::>>>'+JSON.stringify(this.preSelectedItems));
        if (this.preSelectedItems !== null && Array.isArray(this.preSelectedItems)) {
            this.preSelectedItems.forEach((item) => {
                var row = new Object(); 
                row.Id = item.Id;  
                row.Name = item.Name;   
                row.eGrants_Project_Agreement_Number__c = item.eGrants_Project_Agreement_Number__c;
                row.eGrants_Description__c = item.eGrants_Description__c; 
                row.Link = '/'+item.Id;   
                row.selected = true;
                this.selectedRecords.push(row); 
            })     
            // console.log('this.selectedRecords in child:::>>>'+JSON.stringify(this.selectedRecords));
          }
        // console.log('this.records in child:::>>>'+JSON.stringify(this.records));
        this.paginationVisibility = this.showPagination === false ? HIDEDIV : SHOWDIV;
        this.filteredRecords = this.records; 
        this.filtredNum = this.totalRecords;
        // console.log('this.mode in child:::>>>'+JSON.stringify(this.mode));
    ...
    setRecordsOnPage(){
        ...
        if(this.preSelectedItems != null && this.preSelectedItems && this.preSelectedItems.length != 0){
            
            this.selectedRecords = this.preSelectedItems;
        }
        this.preSelected = [];
        ...
        if( this.preSelectedItems != null && this.preSelectedItems && this.preSelectedItems.length != 0){
            ...
            this.totalSelected = this.preSelectedItems.length;
        }
        if(this.selectedRecords != null && this.selectedRecords && this.selectedRecords.length > 0){
         
            this.refreshCurrentData = true;
        } 
        if(this.recordsToDisplay != null && this.recordsToDisplay.length == 0){
        ...
        this.dispatchEvent(new CustomEvent('setselectedrecords', {detail: this.selectedRecords})); //Send records to display on table to the parent component
    ...
    handelRowsSelected(selectedRows) {
        ...
        // if(this.maxRowSelection != '1' && this.recordsToDisplay && this.recordsToDisplay?.length > 0 && 
        ...
        //handling edge case where there was only one preselected item.
        if(selectedRows.length == 0 && this.selectedRecords.length == 1){
             console.log('clearing selected Recoreds');
            this.selectedRecords  = [];
        }
        this.selectedRecords = this.mergeObjectArray(this.selectedRecords, this.pageSelectedRecords, "Id"); 
        if(this.maxRowSelection === '1' && selectedRows!=null && selectedRows.length > 0){
        //if(this.maxRowSelection == '1' && selectedRows!=null && selectedRows.length > 0){
    ...
    mergeObjectArray(firstArray, secondArray, prop){
        ...
        // console.log('mergeObjectArray reduced:::>>>'+JSON.stringify(reduced));
        // console.log('mergeObjectArray firstArray:::>>>'+JSON.stringify(firstArray));
        // console.log('mergeObjectArray secondArray:::>>>'+JSON.stringify(secondArray));
